...
TEST_F(GetAPITest, Partial) {
  ...
  EXPECT_EQ(IPCZ_RESULT_UNAVAILABLE,
            ipcz.Get(p, IPCZ_GET_PARTIAL, nullptr, out_data, &num_bytes,
                     &out_handle, &num_handles));
}

TEST_F(GetAPITest, Empty) {
  EXPECT_EQ(
      IPCZ_RESULT_UNAVAILABLE,
      ipcz.Get(q, IPCZ_NO_FLAGS, nullptr, nullptr, nullptr, nullptr, nullptr));
}
...
TEST_F(GetAPITest, Dead) {
  ...
  OpenPortals(&a, &b);
  ClosePortals({b});
  EXPECT_EQ(IPCZ_RESULT_NOT_FOUND, ipcz.Get(a, IPCZ_NO_FLAGS, nullptr, nullptr,
                                            nullptr, nullptr, nullptr));
  ClosePortals({a});
}
